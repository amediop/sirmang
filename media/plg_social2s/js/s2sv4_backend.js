...
function s2sbackend_try(){
	try {
	  ...
	  console.log("s2s_try: jQuery not active. Waitting...");

	  setTimeout(function() { s2sbackend_try() }, 1000);
	}
}


function s2sbackend_init(){
...
if(s2sv3_email != ''){
  ...
  */
    ...
    //disable button
    jQuery('.fa-chess-queen').parent().addClass('disabled').css("pointer-events","none");
    jQuery('[aria-controls="attrib-SOCIAL2S_LICENSE"]').removeClass('disabled').css("pointer-events","all");
  ...
  function s2s_interface_j4(){
    ...
    //ACCORDIONS
    jQuery('.accordion-collapse').each(function(index, value){

      //!
      jQuery(this).siblings('.control-group').children('.control-label').addClass("s2s_group");

      jQuery(this).find('.accordion-body').append(jQuery(this).siblings('.control-group'));
     
      //jQuery(this).unwrap();
    });

    //current version
    //jQuery('.s2s_version ').append(' <span class="badge">db:'+current_version+'</span>');
