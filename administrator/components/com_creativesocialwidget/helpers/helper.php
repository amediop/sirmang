...
class creativesocialwidgetHelper {
	/**
	...
	public static function addSubmenu($title, $v, $controller = null, $image = null) {
		$enabled = false;
		$view = JRequest::getWord("view", 'creativesocialwidget');
		if($view == $v) {
			$img = $v;
			if($image != null) $img = $image;
			JToolBarHelper::title(   JText::_( $title).' - '.( 'Creative Social Widget' ), $img.'.png' );
			$enabled = true;
		}
		$link = 'index.php?option=com_creativesocialwidget&view='.$v;
